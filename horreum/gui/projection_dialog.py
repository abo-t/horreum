...
class ProjectionDialog(QDialog):
    ...
    manifestu `_PROJEKCJA.json`; `dry_async=False` = worker inline (seam testowy, wzorzec
    `_writeback_async` gridu)."""

    def __init__(self, con, frame_ids, *, now_fn, perspektywa="perspektywa", dry_async=True,
                 parent=None):
    ...
    def _build_ui(self):
        ...
        act = QHBoxLayout()
        self.busy = QProgressBar()                   # bieg auto-DRY WIDOCZNY (wiz W2: tekst to za mało)
        self.busy.setRange(0, 0)                     # indeterminate — DRY nie zna liczby z góry
    ...
    def _trigger_dry(self):
        ...
        if self._dry_async and self._db_path:
    ...
    def _on_apply(self):
        root = self._current_root()
        if self._plan is None or self._dry is None or not root:
            return
        QGuiApplication.setOverrideCursor(Qt.WaitCursor)
        try:
            res = projection.apply(
                self._plan, root, do_apply=True, copy=self._dry["copy"], now=self._now(),
        ...
        self._settings().setValue("projection/last_target", root)   # ostatnio UŻYTY cel = domyślny
        self.report.setPlainText(self._format(res, self._plan, dry=False))
        self.btn_apply.setEnabled(False)             # zrobione — kolejne „Utwórz" wymaga nowego DRY
        self.btn_apply.setText("Utworzono ✓")        # przycisk nie głosi już akcji, która zaszła (wiz K2)
    ...
    def _format(self, res, plan, *, dry, partial=False, payload=None):
        ...
        nf = len(folders)
        lines.append(f"  drzewo: {nf} {plural(nf, 'folder', 'foldery', 'folderów')} kategorii")
    ...
    # ---- sprzątanie wątku przy zamknięciu ----
    def done(self, r):
        """Zamknięcie dialogu (accept/reject/X) nie może zostawić żywego QThread pod kasowanym
        rodzicem. `request_cancel` łapie na granicy pliku sondy DRY."""
        if self._dry_worker is not None:
